...
function info ( api )
{
	...
	add( 'cell',   api.cells( { selected: true } ).flatten().length );
       ...
       var add = function ( name, num ) {
		output.append( $('<span class="select-item"/>').append( api.i18n(
			'select.'+name+'s',
			{ _: '%d '+name+'s selezionate', 0: '', 1: '1 '+name+' selezionate' },
			num
		) ) );
	};

	add( 'righe',    api.rows( { selected: true } ).flatten().length );
	add( 'colonne', api.columns( { selected: true } ).flatten().length );
	add( 'celle',   api.cells( { selected: true } ).flatten().length );
       // Fine traduzione
